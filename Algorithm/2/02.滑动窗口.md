...
<!-- 和非固定窗口大小不同，固定窗口大小的第一个窗口的右端点一定是大于等于 k - 1， 其中 k 为窗口大小。而非固定右端点可能更小。

为了代码简单，我们可以先处理第一个窗口， 然后让左端点从 1 右端点从 k 开始，这样逻辑更统一，下面我们举个例子来说明。

假设我们要求窗口大小为 k 情况下的窗口元素和最大为多少。
...
class Solution:
    def solve(self, nums: List[int], k: int) -> int:
        t = ans = l = 0
        # 第一步， 计算初始窗口的信息。 比如这里我们计算的是窗口内的元素出现次数， 并维护窗口的元素和
        ...
        # 右指针不断移动
        for r in range(k, len(nums)):
            t += nums[r]-nums[l] # 更新窗口内信息 1
            counter[nums[l]] -= 1 # 更新窗口内信息 2
            if counter[nums[l]] == 0: del counter[nums[l]] # # 更新窗口内信息 3
            counter[nums[r]] += 1 # # 更新窗口内信息 4
            if len(counter) == k: # 如何没有重复元素则更新答案
                ans = max(ans, t)
            l += 1
...
- [Most Occurring Number After K Increments](https://binarysearch.com/problems/Most-Occurring-Number-After-K-Increments) 贪心 + 双指针
- [Longest Equivalent Sublist After K Increments](https://binarysearch.com/problems/Longest-Equivalent-Sublist-After-K-Increments) 和上面的题目非常类似，推荐一起做
